...
var _ = Describe(
	"Getting ServiceFilterValues via API",
	Label("e2e", "ServiceFilterValues"),
	func() {
		...
		When("the database is empty", func() {
			...
			It("returns empty for userNames", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				e2e_common.ExpectNonSystemUserNames(
					respData.ServiceFilterValues.UserName.Values,
					[]*string{},
				)
			})
			It("returns empty for uniqueUserID", func() {
				...
				Expect(err).ToNot(HaveOccurred())
				e2e_common.ExpectNonSystemUserUniqueUserIds(
					respData.ServiceFilterValues.UniqueUserID.Values,
					[]*string{},
				)
			})
		})
		...
		When("the database has 10 entries", func() {
			...
			Context("and no additional filters are present", func() {
				It("returns correct serviceCcrns", func() {
					...
					Expect(err).ToNot(HaveOccurred())
					Expect(
						len(respData.ServiceFilterValues.ServiceCcrn.Values),
					).To(Equal(len(seedCollection.ServiceRows)))
				...
				It("returns correct supportGroupCcrns", func() {
					...
					Expect(
						len(respData.ServiceFilterValues.SupportGroupCcrn.Values),
				...
				It("returns correct userNames", func() {
					...
					e2e_common.ExpectNonSystemUserCount(
						len(respData.ServiceFilterValues.UserName.Values),
						len(seedCollection.UserRows),
					)
				...
				It("returns correct UniqueUserID", func() {
					...
					e2e_common.ExpectNonSystemUserCount(
						len(respData.ServiceFilterValues.UniqueUserID.Values),
						len(seedCollection.UserRows),
					)

					existingUniqueUserIds := lo.Map(
						seedCollection.UserRows,
						func(s mariadb.UserRow, index int) string {
				...
				It("returns correct Name With Id", func() {
					...
					e2e_common.ExpectNonSystemUserCount(
						len(respData.ServiceFilterValues.User.Values),
						len(seedCollection.UserRows),
					)

					existingDisplays := lo.Map(
						seedCollection.UserRows,
						func(s mariadb.UserRow, index int) string {
							return fmt.Sprintf("%s (%s)", s.Name.String, s.UniqueUserID.String)
						},
					)
					existingValues := lo.Map(
						seedCollection.UserRows,
						func(s mariadb.UserRow, index int) string {
							return s.UniqueUserID.String
						},
