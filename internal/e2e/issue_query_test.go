					...
					It("- returns the expected content", func() {
						...
						for _, issue := range respData.Issues.Edges {
							...
							for _, iv := range issue.Node.IssueVariants.Edges {
								...
								Expect(
									iv.Node.Severity,
								).ToNot(BeNil(), "IssueVariant has a severity set")
								Expect(
									iv.Node.Severity.Score,
								).ToNot(BeNil(), "severity has a score set")
									...
									func(row mariadb.IssueVariantRow) bool {
										return fmt.Sprintf(
											...
											fmt.Sprintf(
												"%d",
												row.IssueId.Int64,
											) == issue.Node.ID && // belongs actually to the issue
											fmt.Sprintf(
												"%d",
												row.IssueRepositoryId.Int64,
											) == iv.Node.IssueRepository.ID // references correct repository
									},
								)
								Expect(
									ivFound,
								).To(BeTrue(), "attached issueVariant does exist and belongs to issue and repository belongs to issueVariant")

								Expect(
									iv.Node.IssueRepository.Name,
								).ToNot(BeNil(), "Repository name is set")
							}
								...
								_, issueMatchFound := lo.Find(
									...
									func(row mariadb.IssueMatchRow) bool {
										return fmt.Sprintf(
											...
											// *vm.Node.TargetRemediationDate && // target
											// remediation date matches
											fmt.Sprintf(
												"%d",
												row.IssueId.Int64,
											) == issue.Node.ID && // issue match belongs to the respective issue
											fmt.Sprintf(
												"%d",
												row.ComponentInstanceId.Int64,
											) == im.Node.ComponentInstance.ID // correct component instance attached to issue match
									},
								)
								Expect(
									issueMatchFound,
								).To(BeTrue(), "attached IssueMatch is correct")

								_, componentInstanceFound := lo.Find(
									seedCollection.ComponentInstanceRows,
									func(row mariadb.ComponentInstanceRow) bool {
										return fmt.Sprintf(
											"%d",
											row.Id.Int64,
										) == im.Node.ComponentInstance.ID &&
											row.CCRN.String == *im.Node.ComponentInstance.Ccrn &&
											int(row.Count.Int16) == *im.Node.ComponentInstance.Count
									},
								)
								Expect(
									componentInstanceFound,
								).To(BeTrue(), "attached Component instance is correct")
							}
						}
					})
					It("- returns the expected PageInfo", func() {
						Expect(
							*respData.Issues.PageInfo.HasNextPage,
						).To(BeTrue(), "hasNextPage is set")
						Expect(
							*respData.Issues.PageInfo.HasPreviousPage,
						).To(BeFalse(), "hasPreviousPage is set")
						Expect(
							respData.Issues.PageInfo.NextPageAfter,
						).ToNot(BeNil(), "nextPageAfter is set")
						Expect(
							len(respData.Issues.PageInfo.Pages),
						).To(Equal(2), "Correct amount of pages")
						Expect(
							*respData.Issues.PageInfo.PageNumber,
			...
			Context("and we request metadata", Label("withObjectMetadata.graphql"), func() {
				It("returns correct metadata counts", func() {
					...
					for _, issueEdge := range respData.Issues.Edges {
						...
						Expect(
							issueEdge.Node.ObjectMetadata.IssueMatchCount,
						).To(Equal(issueEdge.Node.IssueMatches.TotalCount), "IssueMatchCount is correct")
						Expect(
							issueEdge.Node.ObjectMetadata.ComponentInstanceCount,
						).To(Equal(ciCount), "ComponentInstanceCount is correct")
						Expect(
							issueEdge.Node.ObjectMetadata.ServiceCount,
						).To(Equal(len(serviceIdSet)), "ServiceCount is correct")
					}
				})
			})
					...
					By("- returns the expected content in order", func() {
						...
						for _, i := range issues.Edges {
							if len(i.Node.IssueVariants.Edges) > 0 {
								...
								ratings := lo.Map(
									variants,
									func(iv mariadb.IssueVariantRow, _ int) int {
...
var _ = Describe(
	"Modifying relationship of ComponentVersion of Issue via API",
	Label("e2e", "ComponentVersionIssueRelationship"),
	func() {
		var seeder *test.DatabaseSeeder
		var s *server.Server
		var cfg util.Config
		...
		When("the database has 10 entries", func() {
			...
			Context("and a mutation query is performed", func() {
				It("adds componentVersion to issue", Label("addComponentVersion.graphql"), func() {
					...
					componentVersionIds := lo.FilterMap(
						seedCollection.ComponentVersionIssueRows,
						func(row mariadb.ComponentVersionIssueRow, _ int) (int64, bool) {
					...
					_, found := lo.Find(
						respData.Issue.ComponentVersions.Edges,
						func(edge *model.ComponentVersionEdge) bool {
				...
				It(
					"removes componentVersion from issue",
					Label("removeComponentVersion.graphql"),
					func() {
						issue := seedCollection.IssueRows[0].AsIssue()
						...
						respData, err := e2e_common.ExecuteGqlQueryFromFileWithHeaders[struct {
							...
							map[string]any{
								...
								"componentVersionId": fmt.Sprintf(
									"%d",
									componentVersionRow.ComponentVersionId.Int64,
								),
							},
							nil,
						)
