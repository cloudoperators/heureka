...
func createTestIssue(port string) string {
	return createIssue(
		port,
		testIssuePrimaryName,
		testCreatedIssueDescription,
		testCreatedIssueType,
	)
}

func createIssue(port string, primaryName string, description string, itype string) string {
	issue := e2e_common.QueryCreateIssue(
		port,
		e2e_common.Issue{PrimaryName: primaryName, Description: description, Type: itype},
	)
...
func updateTestIssue(port string, iid string) {
	issue := e2e_common.QueryUpdateIssue(
		port,
		e2e_common.Issue{
			PrimaryName: testIssuePrimaryName,
			Description: testUpdatedIssueDescription,
			Type:        testUpdatedIssueType,
		},
		iid,
	)
...
var _ = Describe(
	...
	func() {
		...
		When("New issue is created via API", func() {
			...
			It(
				"shall assign CreatedBy, CreatedAt, UpdatedBy and UpdatedAt metadata fields and shall keep zero value in DeltedAt metadata fields",
				func() {
					...
					Expect(
						*issue.Metadata.CreatedBy,
					).To(Equal(fmt.Sprintf("%d", util.SystemUserId)))
					...
					Expect(
						*issue.Metadata.UpdatedBy,
					).To(Equal(fmt.Sprintf("%d", util.SystemUserId)))
					Expect(updatedAt).To(Equal(createdAt))
					Expect(
						*issue.Metadata.DeletedAt,
					).To(Equal(time.Unix(0, 0).Local().Format(dbDateLayout)))
				},
			)
		})
		When("Issue is updated via API", func() {
			...
			It(
				"shall assign UpdatedBy and UpdatedAt metadata fields and shall keep zero value in DeletedAt metadata field",
				func() {
					Expect(*issue.Description).To(Equal(testUpdatedIssueDescription))
					Expect(issue.Type.String()).To(Equal(testUpdatedIssueType))
		...
		When("Two issues are created and one of them is deleted", func() {
			...
			BeforeEach(func() {
				activeIssueId = createIssue(
					...
					"dummyPrimaryName1",
					"dummyDescription1",
					entity.IssueTypeVulnerability.String(),
				)
				deletedIssueId = createIssue(
					cfg.Port,
					"dummyPrimaryName2",
					"dummyDescription2",
					entity.IssueTypeVulnerability.String(),
			...
			It("shall get one active issue when state filter is specified as 'active'", func() {
				issues := getTestIssuesFilteringByState(
					cfg.Port,
					[]string{model.StateFilterActive.String()},
				)
				Expect(len(issues)).To(Equal(1))
				Expect(issues[0].ID).To(Equal(activeIssueId))
			})
			It("shall get one deleted issue when state filter is specified as 'deleted'", func() {
				issues := getTestIssuesFilteringByState(
					cfg.Port,
					[]string{model.StateFilterDeleted.String()},
				)
				Expect(len(issues)).To(Equal(1))
				Expect(issues[0].ID).To(Equal(deletedIssueId))
			})
			It(
				"shall get both active and deleted issues when state filter is specified as 'active' and 'deleted'",
				func() {
					issues := getTestIssuesFilteringByState(
						cfg.Port,
						[]string{
							model.StateFilterActive.String(),
							model.StateFilterDeleted.String(),
						},
					)
