			...
			Context(
				...
				func() {
					...
					It("- returns the expected content", func() {
						...
						for _, user := range respData.Users.Edges {
							...
							for _, service := range user.Node.Services.Edges {
								...
								Expect(
									serviceFound,
								).To(BeTrue(), "attached service does exist and belongs to user")
							}
					...
					It("- returns the expected PageInfo", func() {
						Expect(
							*respData.Users.PageInfo.HasNextPage,
						).To(BeTrue(), "hasNextPage is set")
						Expect(
							*respData.Users.PageInfo.HasPreviousPage,
						).To(BeFalse(), "hasPreviousPage is set")
						Expect(
							respData.Users.PageInfo.NextPageAfter,
						).ToNot(BeNil(), "nextPageAfter is set")
						Expect(
							len(respData.Users.PageInfo.Pages),
						).To(Equal(3), "Correct amount of pages")
						Expect(
							*respData.Users.PageInfo.PageNumber,
	...
	When("the database has 10 entries", func() {
		...
		Context("and a mutation query is performed", Label("create.graphql"), func() {
			It("creates new user", func() {
				respUser := e2e_common.QueryCreateUser(
					cfg.Port,
					e2e_common.User{
	...
	When("the database has 10 entries", func() {
		...
		Context("and a mutation query is performed", Label("update.graphql"), func() {
			It("updates user", func() {
				...
				user.Name = "Sauron"
				respUser := e2e_common.QueryUpdateUser(
					cfg.Port,
					e2e_common.User{
						UniqueUserID: user.UniqueUserID,
						Name:         user.Name,
