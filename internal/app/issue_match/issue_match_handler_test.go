...
var _ = Describe("When listing IssueMatches", Label("app", "ListIssueMatches"), func() {
	...
	When("the list option does include the totalCount", func() {
		...
		BeforeEach(func() {
			...
			db.On("GetIssueMatches", filter, mock.Anything).Return([]entity.IssueMatchResult{}, nil)
	...
	When("the list option does include the PageInfo", func() {
		...
		DescribeTable("pagination information is correct", func(pageSize int, dbElements int, resElements int, hasNextPage bool) {
			...
			imr := lo.Map(matches, func(m entity.IssueMatch, _ int) entity.IssueMatchResult {
				return entity.IssueMatchResult{
					WithCursor: entity.WithCursor{Value: fmt.Sprintf("id=%d", m.Id)},
					IssueMatch: &m,
				}
			})

			var ids = lo.Map(imr, func(m entity.IssueMatchResult, _ int) string { return *m.Cursor() })
			var i int64 = 0
			for len(ids) < dbElements {
				i++
				ids = append(ids, fmt.Sprintf("id=%d", i))
			}
			db.On("GetIssueMatches", filter, mock.Anything).Return(imr, nil)
			db.On("GetAllIssueMatchCursors", filter, mock.Anything).Return(ids, nil)
			issueMatchHandler = im.NewIssueMatchHandler(db, er, nil)
			res, err := issueMatchHandler.ListIssueMatches(filter, options)
			Expect(err).To(BeNil(), "no error should be thrown")
	...
	When("the list options does NOT include aggregations", func() {
		...
		Context("and the filter does have results in the database", func() {
			BeforeEach(func() {
				matches := test.NNewFakeIssueMatches(15)
				imr := lo.Map(matches, func(m entity.IssueMatch, _ int) entity.IssueMatchResult {
					return entity.IssueMatchResult{
						WithCursor: entity.WithCursor{Value: fmt.Sprintf("id=%d", m.Id)},
						IssueMatch: &m,
		...
		Context("and the database operations throw an error", func() {
			BeforeEach(func() {
				db.On("GetIssueMatches", filter, mock.Anything).Return([]entity.IssueMatchResult{}, errors.New("some error"))
			})
...
var _ = Describe("When updating IssueMatch", Label("app", "UpdateIssueMatch"), func() {
	...
	It("updates issueMatch", func() {
		...
		db.On("GetIssueMatches", filter, mock.Anything).Return([]entity.IssueMatchResult{entity.IssueMatchResult{
			WithCursor: entity.WithCursor{Value: "id=1"},
		...
		Context("when BuildIssueVariantMap succeeds", func() {
			...
			Context("when issue matches already exist", func() {
				BeforeEach(func() {
					...
					db.On("GetIssueMatches", mock.Anything, mock.Anything).Return([]entity.IssueMatchResult{
						{
							WithCursor: entity.WithCursor{Value: fmt.Sprintf("id=%d", issueMatch.Id)},
							IssueMatch: &issueMatch,
						},
					}, nil).Once()
				})
