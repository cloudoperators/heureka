...
func (apt *autoPatchTest) Run(
	tag string,
	info autoPatchTestInfo,
	fn func(db *mariadb.SqlDatabase) (bool, error),
) {
	...
	Expect(
		len(info.expectedResults),
	).To(BeEquivalentTo(len(scannerRuns)), "Number of expected results should be equal to number of scans (%s)", info.description)

	err := scannerRunsSeeder.SeedDbSeeds(info.dbSeeds)
	Expect(err).To(BeNil())

	// Expect ComponentVersionIssues not to be deleted yet
	apt.ExpectComponentVersionIssuesToBeStoredInDatabase(
		info.expectDeletedComponentVersionIssues,
		info.description,
	)
	...
	for i, sr := range scannerRuns {
		...
		Expect(
			res,
		).To(BeEquivalentTo(info.expectedResults[i]), fmt.Sprintf("%s THEN it should return %t (for scan number: %d)", info.description, info.expectedResults[i], i))
	}
	...
	apt.ExpectComponentVersionIssuesToBeDeleted(
		info.expectDeletedComponentVersionIssues,
		info.description,
	)
...
func (apt *autoPatchTest) ExpectPatchForComponentInstance(
	...
	Expect(
		err,
	).To(BeNil(), "%s THEN %s component instance should have one matching patch (%s)", when, componentInstanceName, err)
	Expect(
		patches,
	).NotTo(BeEmpty(), "%s THEN %s component should have at least one patch found", when, componentInstanceName)
}

func (apt *autoPatchTest) ExpectComponentInstanceToBeDeleted(
	componentInstanceName string,
	when string,
) {
	...
	Expect(
		err,
	).To(BeNil(), "%s THEN %s component should be deleted (%s)", when, componentInstanceName, err)
	Expect(
		component.DeletedAt.Valid,
	).To(BeEquivalentTo(true), "%s THEN %s component should be deleted", when, componentInstanceName)
	Expect(
		component.DeletedAt.Time.IsZero(),
	).To(BeEquivalentTo(false), "%s THEN %s component should be deleted", when, componentInstanceName)
}

func (apt *autoPatchTest) ExpectIssueMatchesToBeDeleted(
	expectDeletedIssueMatchesByIssueName []string,
	when string,
) {
	...
	Expect(
		err,
	).To(BeNil(), "%s THEN issue matches: '%s' should be deleted (%s)", when, strings.Join(expectDeletedIssueMatchesByIssueName, ", "), err)
	Expect(
		len(deletedIssues),
	).To(BeEquivalentTo(len(expectDeletedIssueMatchesByIssueName)), "%s THEN issue matches: '%s' should be deleted", when, strings.Join(expectDeletedIssueMatchesByIssueName, ", "))
	Expect(
		deletedIssues,
	).To(ConsistOf(expectDeletedIssueMatchesByIssueName), "%s THEN issue matches: '%s' should be deleted", when, strings.Join(expectDeletedIssueMatchesByIssueName, ", "))
}
...
func (apt *autoPatchTest) ExpectVersionsToBeDeleted(expectDeletedVersions []string, when string) {
	...
	Expect(
		err,
	).To(BeNil(), "%s THEN versions: '%s' should be deleted (%s)", when, strings.Join(expectDeletedVersions, ", "), err)
	Expect(
		len(deletedVersions),
	).To(BeEquivalentTo(len(expectDeletedVersions)), "%s THEN versions: '%s' should be deleted", when, strings.Join(expectDeletedVersions, ", "))
	Expect(
		deletedVersions,
	).To(ConsistOf(expectDeletedVersions), "%s THEN versions: '%s' should be deleted", when, strings.Join(expectDeletedVersions, ", "))
}

func (apt *autoPatchTest) ExpectComponentsToBeDeleted(
	expectDeletedComponents []string,
	when string,
) {
	...
	Expect(
		err,
	).To(BeNil(), "%s THEN components: '%s' should be deleted (%s)", when, strings.Join(expectDeletedComponents, ", "), err)
	Expect(
		len(deletedComponents),
	).To(BeEquivalentTo(len(expectDeletedComponents)), "%s THEN components: '%s' should be deleted", when, strings.Join(expectDeletedComponents, ", "))
	Expect(
		deletedComponents,
	).To(ConsistOf(expectDeletedComponents), "%s THEN components: '%s' should be deleted", when, strings.Join(expectDeletedComponents, ", "))
}

func (apt *autoPatchTest) ExpectComponentVersionIssuesToBeStoredInDatabase(
	expectComponentVersionIssues []test.ComponentVersionIssue,
	when string,
) {
...
func (apt *autoPatchTest) ExpectComponentVersionIssuesToBeDeleted(
	...
	Expect(
		err,
	).To(BeNil(), "%s THEN component version issues: '%s' should be deleted (%s)", when, strings.Join(
		lo.Map(
			expectDeletedComponentVersionIssues,
			func(cvi test.ComponentVersionIssue, _ int) string {
				return fmt.Sprintf("i:%s cv:%s", cvi.Issue, cvi.ComponentVersion)
			},
		),
		", ",
	...
	Expect(
		componentVersionIssues,
	).To(test.ContainNone(expectDeletedComponentVersionIssues), "%s THEN component version issues: '%s' should be deleted", when, strings.Join(
		lo.Map(
			expectDeletedComponentVersionIssues,
			func(cvi test.ComponentVersionIssue, _ int) string {
				return fmt.Sprintf("i:%s cv:%s", cvi.Issue, cvi.ComponentVersion)
...
var _ = Describe("Autopatch", Label("database", "Autopatch"), func() {
	...
	autopatchTests := []autoPatchTestInfo{
		...
		},
		{
			...
			dbSeeds: test.DbSeeds{
				IssueMatchComponents: []test.IssueMatchComponent{
					{Issue: "Issue1", ComponentInstance: "CI1"},
				},
		...
		},
		{
			...
			expectDeletedVersions:                new([]string{"V1"}),
		},
		{
			...
			expectDeletedVersions: new([]string{}),
		},
		{
			...
			expectDeletedVersions:     new([]string{"V20"}),
		},
		{
		...
		},
		{
			...
			patchedComponentInstances: []string{"CI2"},
			expectDeletedVersions:     new([]string{"V20"}),
			expectDeletedComponents:   new([]string{}),
		},
		{
			...
			patchedComponentInstances: []string{"CI2"},
			expectDeletedVersions:     new([]string{"V20"}),
			expectDeletedComponents:   new([]string{"C2"}),
		},
		{
