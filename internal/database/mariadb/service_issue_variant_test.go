...
var _ = Describe("ServiceIssueVariant - ", Label("database", "IssueVariant"), func() {
	...
	When("Getting ServiceIssueVariants", Label("GetServiceIssueVariants"), func() {
		...
		// We need to setup a whole bunch of data to test the filtering for service issue variants
		// based by component instances
		...
		DescribeTable(
			...
			func(filterForInstances int, totalInstances int, totalIssues int) {
				...
				for i := range totalInstances {
					...
					// we do this until it got successfully created to avoid failures through unique
					...
					componentInstances := seeder.SeedComponentInstances(
						1,
						componentVersions,
						services,
					)
					...
					for idx, issue := range issues {
						for irdx, ir := range issueRepositories {
							variants := []string{
								fmt.Sprintf("GHSA-%d", i),
								fmt.Sprintf("RSHA-%d", i),
								fmt.Sprintf("VMSA-%d", i),
							}
				...
				By(
					fmt.Sprintf(
						"having in total %d component instances with each %d issues across the repositories",
						filterForInstances,
						totalIssues,
					),
					func() {
						By("and filtering for this component instance", func() {
							By("it can perform the query correctly", func() {
								// get instance ids to filter for based on count of instances that
								// we want to filter for
								cids := lo.Map(
									allCI,
									func(item mariadb.ComponentInstanceRow, _ int) *int64 { return new(item.Id.Int64) },
								)
								...
								By("and returning all the issue variants", func() {
									Expect(
										len(res),
									).To(BeIdenticalTo((len(issueVariants) / totalInstances) * filterForInstances))
								})
							})
						})
					},
		...
		// Testing issues
		Context(
			"When filtering by IssueId",
			Label("GetServiceIssueVariants", "IssueFilter"),
			func() {
				Context("and the database is empty", func() {
				...
				Context("and there is a single issue with variants", func() {
					...
					It("returns all variants when filtering for the issue", func() {
						filter := &entity.ServiceIssueVariantFilter{
							ComponentInstanceId: []*int64{new(componentInstances[0].Id.Int64)},
