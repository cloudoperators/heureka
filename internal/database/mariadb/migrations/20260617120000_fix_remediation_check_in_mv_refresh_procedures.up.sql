...
-- Replaces the LEFT JOIN + (IS NULL OR expired) pattern with NOT EXISTS so that
-- multiple active remediations for the same (service, issue) are handled correctly.
-- A vulnerability is only excluded when at least one non-deleted, non-expired
-- (or open-ended) remediation exists for it.
