...
func NewProcessor(cfg Config) *Processor {
	gClient := graphql.NewClient(cfg.HeurekaUrl, &http.Client{Timeout: time.Duration(10) * time.Second, Transport: &authedTransport{wrapped: http.DefaultTransport, token: cfg.HeurekaApiToken}})
	return &Processor{
		GraphqlClient:       gClient,
